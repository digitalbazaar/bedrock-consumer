...
bedrock.events.on('bedrock-passport.authenticate', (info, callback) => {
  var config = bedrock.config.consumer;
  var identity = info.user.identity;
  async.auto({
    isPersistent: callback => brIdentity.exists(null, identity.id, callback),
    addDefaults: ['isPersistent', (callback, results) => {
      ...
      info.user.identity = _.assign(
        {}, _.cloneDeep(config.defaults.identity), identity);

      // handle `generateResource` feature
      var roles =
        bedrock.jsonld.getValues(info.user.identity, 'sysResourceRole');
      for(var i = 0; i < roles.length; ++i) {
        if(roles[i].generateResource === 'id') {
          roles[i] = brIdentity.generateResource(roles[i], identity.id);
        }
      }
      callback();
    }]
